...
public class WriteBenchmark {
    ...
    @Benchmark
    public int kervixBatch(WriteState state) {
        int total = 0;
        for (int count : state.kervix.insertBatch(state.batchRows)) {
            total += count;
        }
        return total;
    }
    ...
    @Benchmark public long kervixGeneratedKey(WriteState state) { return state.kervix.insertGenerated("generated"); }
    ...
    public static class WriteState {
        ...
        public void setup() throws Exception {
            ...
            kervix = new KervixBenchmarkMapperImpl(Kervix.jdbc(dataSource).build().sqlExecutor());
