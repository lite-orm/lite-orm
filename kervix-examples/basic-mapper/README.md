# Kervix Basic Mapper Example

This module is an executable Maven consumer of `kervix-core`. It demonstrates:
...
```bash
mvn -pl kervix-examples/basic-mapper -am clean test
```

Generated sources are written to:

```text
kervix-examples/basic-mapper/target/generated-sources/annotations
```
...
The XML equivalent uses `<batch id="insertBatch">`. Kervix generates the per-item parameter loop as Java source; it does not resolve collection paths through a runtime expression engine.
