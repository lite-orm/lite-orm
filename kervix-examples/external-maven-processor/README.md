...
`kervix-processor` Maven Invoker verification.

It is intentionally not listed in the repository root `<modules>` section.
During `mvn -pl kervix-core verify`, Maven Invoker:

1. installs the built `kervix-core` and `kervix-processor` artifacts into an isolated repository;
...
5. confirms that `KervixProcessor` generates `ExternalUserMapperImpl` and that
