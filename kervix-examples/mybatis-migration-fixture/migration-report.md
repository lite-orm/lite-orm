...
| --- | --- | --- | --- | --- |
| `UserMapper#getUser` and its static XML statement | done | Kervix compatibility matrix | Convert XML-backed method to `@Mapper`/`@Select` with explicit `@Param` | Independent Maven `clean verify`; generated `UserMapperImpl` |
...
| Location | Construct | Matrix status | Kervix action | Evidence |
| --- | --- | --- | --- | --- |
| `migration/before/.../UserMapper.java` | Mapper interface with one XML-backed method | Deterministic conversion | Add Kervix `@Mapper` and move the static statement to `@Select` | [Mapper API](../../docs/reference/mybatis-compatibility.md#mapper-api-and-sql-sources) |
...
- `org.mybatis.spring.sample.mapper.UserMapper` now uses Kervix annotations.
...
- Database execution is not claimed by this compile-time fixture. The relevant
  PostgreSQL/MySQL behavior remains covered by Kervix's existing Testcontainers
  suites.
...
mvn -f kervix-examples/mybatis-migration-fixture/pom.xml clean verify
```

Observed on September 13, 2026 from the repository root with OpenJDK 21.0.4:

- `mvn -pl kervix-processor -am -DskipTests install`: exit `0`.
- `mvn -f kervix-examples/mybatis-migration-fixture/pom.xml clean verify`: exit
