...
public class WriteBenchmark {
    ...
    @Benchmark
    public int liteOrmBatch(WriteState state) {
        int total = 0;
        for (int count : state.liteOrm.insertBatch(state.batchRows)) {
            total += count;
        }
        return total;
    }
    ...
    @Benchmark public long liteOrmGeneratedKey(WriteState state) { return state.liteOrm.insertGenerated("generated"); }
    ...
    public static class WriteState {
        ...
        public void setup() throws Exception {
            ...
            liteOrm = new LiteOrmBenchmarkMapperImpl(LiteOrm.jdbc(dataSource).build().sqlExecutor());
