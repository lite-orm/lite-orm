...
@Mapper
public interface MySqlTypesMapper {

    @Insert("INSERT INTO mysql_type_values (id, uuid_value) VALUES (#{id}, #{value})")
    int insertUuid(@Param("id") long id, @Param("value") UUID value);

    @Select("SELECT uuid_value FROM mysql_type_values WHERE id = #{id}")
    UUID findUuid(@Param("id") long id);

    @Select("SELECT uuid_value FROM mysql_type_values ORDER BY id")
    List<UUID> findUuids();

    @Select("SELECT uuid_value FROM mysql_type_values WHERE id = #{id}")
    Optional<UUID> findOptionalUuid(@Param("id") long id);

    @Insert("INSERT INTO mysql_type_values (id, local_time_value) VALUES (#{id}, #{value})")
    int insertLocalTime(@Param("id") long id, @Param("value") LocalTime value);

    @Select("SELECT local_time_value FROM mysql_type_values WHERE id = #{id}")
    LocalTime findLocalTime(@Param("id") long id);

    @Select("SELECT local_time_value FROM mysql_type_values ORDER BY id")
    List<LocalTime> findLocalTimes();

    @Select("SELECT local_time_value FROM mysql_type_values WHERE id = #{id}")
    Optional<LocalTime> findOptionalLocalTime(@Param("id") long id);

    @Insert("INSERT INTO mysql_type_values (id, offset_date_time_value) VALUES (#{id}, #{value})")
    int insertOffsetDateTime(@Param("id") long id, @Param("value") OffsetDateTime value);

    @Select("SELECT offset_date_time_value FROM mysql_type_values WHERE id = #{id}")
    OffsetDateTime findOffsetDateTime(@Param("id") long id);

    @Select("SELECT offset_date_time_value FROM mysql_type_values ORDER BY id")
    List<OffsetDateTime> findOffsetDateTimes();

    @Select("SELECT offset_date_time_value FROM mysql_type_values WHERE id = #{id}")
    Optional<OffsetDateTime> findOptionalOffsetDateTime(@Param("id") long id);

    @Insert("INSERT INTO mysql_type_values "
    ...
    @Select("SELECT id, uuid_value, local_time_value, offset_date_time_value "
        + "FROM mysql_type_values WHERE id = #{id}")
    MySqlTypesRecord findRecord(@Param("id") long id);

    @Select("SELECT id, uuid_value, local_time_value, offset_date_time_value "
        + "FROM mysql_type_values WHERE id = #{id}")
    MySqlTypesBean findBean(@Param("id") long id);
}
