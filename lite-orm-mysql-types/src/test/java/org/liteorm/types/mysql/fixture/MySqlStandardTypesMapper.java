...
@Mapper
public interface MySqlStandardTypesMapper {
    ...
    @Insert("INSERT INTO mysql_standard_type_values (id, integer_value, binary_value) "
    ...
    @Select("SELECT integer_value FROM mysql_standard_type_values WHERE id = #{id}")
    BigInteger findInteger(@Param("id") long id);

    @Select("SELECT integer_value FROM mysql_standard_type_values "
    ...
    @Insert("<script>INSERT INTO mysql_standard_type_values (id, integer_value) "
        + "<if test=\"value != null\">VALUES (#{id}, #{value})</if></script>")
    int insertIntegerDynamic(@Param("id") long id, @Param("value") BigInteger value);

    @Batch("INSERT INTO mysql_standard_type_values (id, integer_value) "
        + "VALUES (#{item.id}, #{item.value})")
    int[] insertIntegerBatch(List<IntegerRow> rows);

    @Insert("INSERT INTO mysql_standard_type_values (id, integer_value, enum_ordinal_value) "
    ...
    @Select("SELECT integer_value AS integerValue, enum_ordinal_value AS enumOrdinalValue "
        + "FROM mysql_standard_type_values WHERE id = #{id}")
    ...
    @GeneratedKey("id")
    @Insert("INSERT INTO mysql_standard_generated_keys (description) VALUES (#{description})")
    BigInteger insertGeneratedKey(@Param("description") String description);

    @Select("SELECT binary_value FROM mysql_standard_type_values WHERE id = #{id}")
    Byte[] findBinary(@Param("id") long id);

    @Insert("INSERT INTO mysql_standard_type_values "
    ...
    @Select("SELECT util_date_value FROM mysql_standard_type_values WHERE id = #{id}")
    java.util.Date findUtilDate(@Param("id") long id);

    @Select("SELECT sql_date_value FROM mysql_standard_type_values WHERE id = #{id}")
    java.sql.Date findSqlDate(@Param("id") long id);

    @Select("SELECT sql_time_value FROM mysql_standard_type_values WHERE id = #{id}")
    java.sql.Time findSqlTime(@Param("id") long id);

    @Select("SELECT sql_timestamp_value FROM mysql_standard_type_values WHERE id = #{id}")
    ...
    @Select("SELECT year_value FROM mysql_standard_type_values WHERE id = #{id}")
    Year findYear(@Param("id") long id);

    @Select("SELECT month_value FROM mysql_standard_type_values WHERE id = #{id}")
    Month findMonth(@Param("id") long id);

    @Select("SELECT year_month_value FROM mysql_standard_type_values WHERE id = #{id}")
    YearMonth findYearMonth(@Param("id") long id);

    @Select("SELECT japanese_date_value FROM mysql_standard_type_values WHERE id = #{id}")
    JapaneseDate findJapaneseDate(@Param("id") long id);

    @Insert("INSERT INTO mysql_standard_type_values (id, enum_name_value, enum_ordinal_value) "
