# Lynxus JMH Benchmarks

This module compares Direct JDBC, generated Lynxus Mappers, and MyBatis using
...
java -jar lynxus-benchmarks/target/benchmarks.jar \
  ...
  -prof gc \
  -rf json \
  -rff lynxus-benchmarks/target/results/core-ga-baseline.json
```
...
java -Dbenchmark.database=mysql \
  -jar lynxus-benchmarks/target/benchmarks.jar \
  '.*Benchmark.*' -rf json \
  -rff lynxus-benchmarks/target/results/mysql.json
```
