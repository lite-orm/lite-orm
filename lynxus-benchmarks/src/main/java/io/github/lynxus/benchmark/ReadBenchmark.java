...
public class ReadBenchmark {
    ...
    @Benchmark public String lynxusScalar(ReadState state) { return state.lynxus.scalar(500L); }
    ...
    @Benchmark public BenchmarkRecord lynxusRecord(ReadState state) { return state.lynxus.record(500L); }
    @Benchmark public BenchmarkBean lynxusBean(ReadState state) { return state.lynxus.bean(500L); }
    ...
    @Benchmark
    public int lynxusDynamic(ReadState state) {
        return state.lynxus.dynamic("user-520", 30).size();
    }
    ...
    @Benchmark
    public long lynxusCursor(ReadState state) {
        return state.lynxus.scan(991L, cursor -> {
            long sum = 0;
            while (cursor.next()) {
    ...
    @Benchmark
    public BenchmarkRecord lynxusNoopInterceptor(ReadState state, Blackhole blackhole) {
        BenchmarkRecord row = state.lynxusObserved.record(500L);
    ...
    public static class ReadState {
        ...
        LynxusBenchmarkMapper lynxusObserved;
        ...
        public void setup() throws SQLException {
            ...
            JdbcAssembly observed = Lynxus.jdbc(dataSource)
                .interceptors(java.util.List.of(new ExecutionInterceptor() { }))
                .build();
            lynxus = new LynxusBenchmarkMapperImpl(assembly.sqlExecutor());
            lynxusObserved = new LynxusBenchmarkMapperImpl(observed.sqlExecutor());
