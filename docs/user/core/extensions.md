...
| --- | --- | --- |
| Reusable Java/JDBC value representation | `JdbcTypeMappings` and `JdbcValueAdapter<T>` | Mapper package, reads and writes |
...
JDBC type mappings, parameter binders, providers, and row mappers are validated during compilation and directly referenced by generated code. They are not discovered per call. Runtime execution still belongs to `JdbcSqlExecutor`.
