...
This document is the single compatibility classification for Kervix. It compares Kervix with
...
| Status | Meaning |
| --- | --- |
| Direct support | Kervix has a documented contract and generated/runtime evidence for the behavior. |
| Deterministic conversion | A supported MyBatis shape can be rewritten into Kervix declarations without runtime interpretation. |
...
| Explicit rejection | Kervix rejects the behavior or deliberately does not provide its MyBatis runtime equivalent. |
...
- [JDBC compatibility fixtures](../../kervix-processor/src/test/java/org/kervix/test/database)
- [XML compiler diagnostics](../../kervix-processor/src/test/java/org/kervix/test/UnsupportedMapperSignatureCompilationTest.java)
- [Generated-source tests](../../kervix-processor/src/test/java/org/kervix/test/generated)
- [External Maven consumer fixture](../../kervix-examples/external-maven-processor)
- [External Gradle consumer fixture](../../kervix-examples/external-gradle-processor)
...
| MyBatis capability | Status | Kervix boundary and migration action | Evidence |
| --- | --- | --- | --- |
| Mapper interfaces and CRUD methods | Deterministic conversion | Replace MyBatis annotation imports with `io.github.kervix.annotation` declarations. Generated implementations are ordinary classes constructed with `SqlExecutor`. | [Core contract](core-contract.md), [migration guide](../user/migration/from-mybatis.md) |
| `@Select`, `@Insert`, `@Update`, `@Delete` | Direct support | Use the Kervix equivalents. Static SQL and the controlled script subset compile into Java. | [Core contract](core-contract.md#21-sql-sources) |
...
| `@Options` and arbitrary statement options | Explicit rejection | Kervix currently emits default options from generated Mappers. Custom plan construction may set the documented JDBC options; no Mapper `@Options` contract exists. | [Core contract](core-contract.md#28-statement-options-and-pagination) |
...
| `select`, `insert`, `update`, `delete`, and `batch` declarations | Direct support | Use the controlled top-level subset. IDs must be non-blank and unique within the resource. | [Core contract](core-contract.md#21-sql-sources), [XML diagnostics](../../kervix-processor/src/test/java/org/kervix/test/UnsupportedMapperSignatureCompilationTest.java) |
...
| `sql` and `include` fragments | Direct support | Fragment IDs and references are validated across the complete resource. Missing and cyclic references fail compilation. | [Core contract](core-contract.md#21-sql-sources), [XML diagnostics](../../kervix-processor/src/test/java/org/kervix/test/UnsupportedMapperSignatureCompilationTest.java) |
| XML namespaces and declaration integrity | Deterministic conversion | `<mapper namespace>` must equal the fully qualified Mapper name. Unsupported top-level tags, attributes, malformed declarations, and invalid unused declarations fail compilation. | [Core contract](core-contract.md#21-sql-sources), [XML diagnostics](../../kervix-processor/src/test/java/org/kervix/test/UnsupportedMapperSignatureCompilationTest.java) |
| `${}` substitution | Explicit rejection | Use bound `#{}` values or a provider for validated SQL structure. Kervix never treats unsafe substitution as plain text. | [Core contract](core-contract.md#21-sql-sources), [migration guide](../user/migration/from-mybatis.md#4-replace-runtime-ognl-assumptions) |
...
| Flat `resultMap` declarations | Deterministic conversion | Use scalar mappings, JavaBean `<id>` / `<result>`, or record `<constructor>` arguments. All declarations and references are validated at compile time. | [Core contract](core-contract.md#24-result-mapping), [XML diagnostics](../../kervix-processor/src/test/java/org/kervix/test/UnsupportedMapperSignatureCompilationTest.java) |
...
| JDBC batch execution | Direct support | Use Kervix `@Batch` or XML `<batch>` with one `List<T>` argument and the driver-provided counts. | [Core contract](core-contract.md#23-return-shapes) |
...
| MyBatis built-in JDBC value handlers | Deterministic conversion | Use Kervix's fixed Core routes. The supported Java/JDBC matrix and database evidence live in the Core contract. | [Core contract](core-contract.md#25-standard-jdbc-type-routing), [Core contract](core-contract.md#26-built-in-jdbc-types) |
...
| Advanced propagation, savepoints, distributed transactions, and recovery | Explicit rejection | Delegate host transaction policy to Spring or another transaction system. Kervix does not coordinate distributed commits. | [Core contract](core-contract.md#5-standalone-transaction-contract) |
| First/second-level cache | Explicit rejection | Use an application cache outside Kervix. | [Design philosophy](../../Design-Philosophy.md#explicit-non-goals) |
...
2. Use a typed provider, binder, row mapper, interceptor, or Spring adapter only when the corresponding Kervix contract owns the behavior.
