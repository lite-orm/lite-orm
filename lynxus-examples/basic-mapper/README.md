# Lynxus Basic Mapper Example

This module is an executable Maven consumer of `lynxus-core`. It demonstrates:
...
- Core standard JDBC type routing;
...
```bash
mvn -pl lynxus-examples/basic-mapper -am clean test
```

Generated sources are written to:

```text
lynxus-examples/basic-mapper/target/generated-sources/annotations
```
...
The XML equivalent uses `<batch id="insertBatch">`. Lynxus generates the per-item parameter loop as Java source; it does not resolve collection paths through a runtime expression engine.
