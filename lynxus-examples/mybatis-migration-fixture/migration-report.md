...
| --- | --- | --- | --- | --- |
| `UserMapper#getUser` and its static XML statement | done | Lynxus compatibility matrix | Convert XML-backed method to `@Mapper`/`@Select` with explicit `@Param` | Independent Maven `clean verify`; generated `UserMapperImpl` |
...
| Location | Construct | Matrix status | Lynxus action | Evidence |
| --- | --- | --- | --- | --- |
| `migration/before/.../UserMapper.java` | Mapper interface with one XML-backed method | Deterministic conversion | Add Lynxus `@Mapper` and move the static statement to `@Select` | [Mapper API](../../docs/reference/mybatis-compatibility.md#mapper-api-and-sql-sources) |
...
- `org.mybatis.spring.sample.mapper.UserMapper` now uses Lynxus annotations.
...
mvn -f lynxus-examples/mybatis-migration-fixture/pom.xml clean verify
```

Observed on September 13, 2026 from the repository root with OpenJDK 21.0.4:

- `mvn -pl lynxus-processor -am -DskipTests install`: exit `0`.
- `mvn -f lynxus-examples/mybatis-migration-fixture/pom.xml clean verify`: exit
