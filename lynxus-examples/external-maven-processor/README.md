...
`lynxus-processor` Maven Invoker verification.

It is intentionally not listed in the repository root `<modules>` section.
During `mvn -pl lynxus-core verify`, Maven Invoker:

1. installs the built `lynxus-core` and `lynxus-processor` artifacts into an isolated repository;
...
5. confirms that `LynxusProcessor` generates `ExternalUserMapperImpl` and that
