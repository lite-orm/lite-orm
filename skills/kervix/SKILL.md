---
name: kervix
description: Plan, integrate, build, troubleshoot, or migrate a Java project with Kervix. Use when an AI agent needs to adopt Kervix, configure Mapper/DataSource boundaries, generate a bounded implementation TODO list, or migrate MyBatis; scan first and pause for user decisions when scope or semantics are unclear.
---

# Kervix Agent Skill

Use this skill as the default agent workflow for Kervix work. It covers greenfield adoption, existing-project integration, verification, and the MyBatis migration path. Keep changes incremental, reviewable, and tied to Kervix's documented contracts.

## Operating model

Kervix work is agent-led but user-directed. A large repository is never an excuse to guess or to rewrite everything in one pass. Inspect first, produce a bounded TODO list, complete one batch, report evidence, and ask before crossing an authorization or semantic decision gate.

## Guardrails

- Establish the project path, revision, authorization to edit, Kervix revision, and requested output before reading or changing files outside the current workspace.
- Read the Kervix checkout's `AGENTS.md`, `docs/README.md`, `docs/reference/core-contract.md`, `docs/reference/extensions.md`, and the relevant user guide before making contract claims.
...
- Kervix checkout and contract revision;
...
```bash
python3 skills/kervix/scripts/scan_mybatis.py \
...
1. Pin the MyBatis version and Kervix contract revision.
...
The migration skill is therefore a mode of this unified `kervix` skill, not a separate user-facing workflow.
