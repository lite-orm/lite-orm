---
name: liteorm
description: Plan, integrate, build, troubleshoot, or migrate a Java project with LiteORM. Use when an AI agent needs to adopt LiteORM, configure Mapper/DataSource boundaries, generate a bounded implementation TODO list, or migrate MyBatis; scan first and pause for user decisions when scope or semantics are unclear.
---

# LiteORM Agent Skill

Use this skill as the default agent workflow for LiteORM work. It covers greenfield adoption, existing-project integration, verification, and the MyBatis migration path. Keep changes incremental, reviewable, and tied to LiteORM's documented contracts.

## Operating model

LiteORM work is agent-led but user-directed. A large repository is never an excuse to guess or to rewrite everything in one pass. Inspect first, produce a bounded TODO list, complete one batch, report evidence, and ask before crossing an authorization or semantic decision gate.

## Guardrails

- Establish the project path, revision, authorization to edit, LiteORM revision, and requested output before reading or changing files outside the current workspace.
- Read the LiteORM checkout's `AGENTS.md`, `docs/README.md`, `docs/reference/core-contract.md`, `docs/reference/extensions.md`, and the relevant user guide before making contract claims.
...
- LiteORM checkout and contract revision;
...
1. Pin the MyBatis version and LiteORM contract revision.
...
The migration skill is therefore a mode of this unified `liteorm` skill, not a separate user-facing workflow.
