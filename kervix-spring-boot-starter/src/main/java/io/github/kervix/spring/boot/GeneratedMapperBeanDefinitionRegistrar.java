...
final class GeneratedMapperBeanDefinitionRegistrar
    ...
    public void postProcessBeanDefinitionRegistry(BeanDefinitionRegistry registry) {
        List<KervixProperties.MapperBinding> bindings = Binder.get(environment)
            .bind(
                "kervix.mapper-bindings",
                Bindable.listOf(KervixProperties.MapperBinding.class)
            )
            .orElse(List.of());
        validateBindings(registry, bindings);
        for (KervixProperties.MapperBinding binding : bindings) {
    ...
    private void registerGeneratedMappers(
            ...
            KervixProperties.MapperBinding binding) {
        ...
        try {
            ...
            if (mapperInterface == null) {
                throw new BeanDefinitionStoreException(
                    "Invalid generated Kervix mapper " + className + ": no @Mapper interface is implemented");
            }
            if (!hasSqlExecutorConstructor(implementationClass)) {
                throw new BeanDefinitionStoreException(
                    "Invalid generated Kervix mapper " + className + ": missing public SqlExecutor constructor");
            }
            ...
            if (registry.containsBeanDefinition(beanName)) {
                ...
                throw new BeanDefinitionStoreException(
                    "Duplicate Kervix mapper bean '" + beanName + "': " + className
            ...
            throw new IllegalStateException("Failed to load generated Kervix mapper " + className, e);
        }
    }

    private void validateBindings(
            BeanDefinitionRegistry registry,
            List<KervixProperties.MapperBinding> bindings) {
        for (int index = 0; index < bindings.size(); index++) {
            KervixProperties.MapperBinding binding = bindings.get(index);
    ...
    private String registerExecutor(BeanDefinitionRegistry registry, String dataSourceName) {
        String executorBeanName = "kervixSqlExecutor#" + dataSourceName;
    ...
    private void requireText(String value, String property) {
        if (value == null || value.isBlank()) {
            throw new BeanDefinitionStoreException(
                "Kervix property '" + property + "' must not be blank");
        }
    }
    ...
    private Class<?> findMapperInterface(Class<?> implementationClass) {
        ...
        for (Class<?> implementedInterface : implementationClass.getInterfaces()) {
            if (implementedInterface.isAnnotationPresent(io.github.kervix.annotation.Mapper.class)) {
